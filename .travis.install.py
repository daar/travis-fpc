#!/usr/bin/env python
# Part of `travis-freepascal` (https://github.com/nielsAD/travis-freepascal)
# License: MIT
...
FPC_TMP_DIR=os.environ.get('FPC_TMP_DIR') or 'freepascal_tmp'
FPC_REL_DEF=os.environ.get('FPC_REL_DEF') or {'linux':'amd64', 'qemu-arm':'amd64', 'qemu-arm-static':'amd64', 'osx':'i386', 'wine':'32'}
FPC_BIN_SRC=os.environ.get('FPC_BIN_SRC') or 'http://mirrors.iwi.me/freepascal/releases/%(target)s/Lazarus%%20%(version)s'
FPC_BIN_TGT=os.environ.get('FPC_BIN_TGT') or {
...
def install_freepascal_default():
    if OS_NAME == 'linux':
        # Make sure nogui is installed for headless runs
        pkg = 'fpc'
    elif OS_NAME == 'osx':
        # Install brew cask first
        pkg = 'fpc caskroom/cask/brew-cask && %s cask install fpcsrc freepascal' % (OS_PMAN)
    else:
        # Default to freepascal
        pkg = 'fpc'
    return os.system('%s install %s' % (OS_PMAN, pkg)) == 0

def install_freepascal_version(ver,rel,env):
    # Download all files in directory for specified Lazarus version
    osn = env or OS_NAME
    tgt = FPC_BIN_TGT[osn] % {'release': rel or FPC_REL_DEF[osn]}
    src = FPC_BIN_SRC % {'target': tgt, 'version': ver}
    if os.system('wget -r -l1 -T 30 -np -nd -nc -A .deb,.dmg,.exe %s -P %s' % (src, FPC_TMP_DIR)) != 0:
        return False
    ...
    if osn == 'wine':
        ...
        process_file = lambda f: (not f.endswith('.exe')) or os.system('wine %s /VERYSILENT /DIR="c:\\freepascal"' % (f)) == 0
    ...
    if not all(map(lambda f: process_file(os.path.join(FPC_TMP_DIR, f)), sorted(os.listdir(FPC_TMP_DIR)))):
        return False

    if osn == 'wine':
        # Set wine Path (persistently) to include Lazarus binary directory
        if os.system('wine cmd /C reg add HKEY_CURRENT_USER\\\\Environment /v PATH /t REG_SZ /d "%PATH%\\;c:\\\\freepascal"') != 0:
            return False

        # Redirect listed executables so they execute in wine
        for alias in ('fpc', 'lazbuild', 'freepascal'):
...
def install_freepascal(ver=None,rel=None,env=None):
    return install_freepascal_default()
    #return install_freepascal_version(ver,rel,env) if ver else install_freepascal_default()

def main():
    os.system('%s update' % (OS_PMAN))
    return install_freepascal(os.environ.get('FPC_VER'),os.environ.get('FPC_REL'),os.environ.get('FPC_ENV'))
